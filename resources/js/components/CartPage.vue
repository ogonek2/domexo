    ...
    <div class="cart-page">
        <div v-if="cart.length > 0" class="cart-page__layout">
                    ...
                    <div class="cart-page__info">
                        ...
                        <div class="cart-page__actions">
                            <div class="pcard__qty cart-page__qty">
                                ...
                                <span class="pcard__qty-value">{{ item.quantity }} шт</span>
            ...
            <aside class="cart-page__summary">
                ...
                <p v-else class="cart-page__notice cart-page__notice--ok">
...
export default {
    ...
    computed: {
        ...
        amountToReachMinimum() {
            const diff = MIN_ORDER_TOTAL - this.totalPrice;
    ...
    methods: {
        ...
        increase(id) {
            ...
            if (item) { item.quantity++; this.saveCart(); }
        },
        decrease(id) {
            const item = this.cart.find(i => i.id == id);
            if (item && item.quantity > 1) { item.quantity--; this.saveCart(); }
        },
        remove(id) {
        ...
        goCheckout() {
            if (this.isBelowMinimum) {
                if (this.$toast) this.$toast.warning('Мінімальна сума замовлення — 1000 ₴');
                return;
            }
...
.cart-page__name a { color: #1E1E1E; text-decoration: none; }
.cart-page__name a:hover { color: #D4AF5A; }
.cart-page__articule { font-size: 0.75rem; color: #888; margin: 0 0 0.5rem; }
.cart-page__price { font-weight: 700; color: #1E1E1E; font-family: Montserrat, sans-serif; }
.cart-page__opt { font-size: 0.6875rem; background: #D4AF5A; color: #1E1E1E; padding: 0.125rem 0.375rem; margin-left: 0.5rem; }
.cart-page__actions { display: flex; align-items: center; gap: 0.75rem; margin-top: 0.75rem; }
.cart-page__qty { max-width: 160px; }
.cart-page__remove { background: none; border: 1px solid #ddd; padding: 0.375rem 0.5rem; color: #666; cursor: pointer; }
.cart-page__remove:hover { border-color: #1E1E1E; color: #1E1E1E; }
.cart-page__subtotal { font-weight: 700; font-family: Montserrat, sans-serif; white-space: nowrap; align-self: start; padding-top: 0.25rem; }
...
.cart-page__summary-title { font-family: Montserrat, sans-serif; font-size: 1.125rem; font-weight: 700; margin: 0 0 1rem; color: #0B1F3B; }
.cart-page__summary-row { display: flex; justify-content: space-between; font-size: 0.875rem; margin-bottom: 0.5rem; color: #666; }
.cart-page__summary-total { font-size: 1.125rem; font-weight: 700; color: #1E1E1E; border-top: 1px solid #eee; padding-top: 0.75rem; margin-top: 0.75rem; margin-bottom: 1rem; }

.cart-page__notice { font-size: 0.8125rem; padding: 0.75rem; margin-bottom: 1rem; border: 1px solid #eee; }
.cart-page__notice--warn { background: #fff8f0; color: #666; border-color: #D4AF5A; }
.cart-page__notice--ok { background: #F5F0E6; color: #1E1E1E; }

.cart-page__checkout {
    display: flex; align-items: center; justify-content: center; gap: 0.5rem;
    width: 100%; padding: 0.875rem;
    background: #D4AF5A; color: #1E1E1E;
    font-weight: 700; font-family: Montserrat, sans-serif;
    text-decoration: none; border: none; cursor: pointer;
    transition: background 0.2s;
}

.cart-page__checkout:hover { background: #C5A059; }
...
.cart-page__continue:hover { color: #D4AF5A; }
...
.cart-page__empty h2 { font-family: Montserrat, sans-serif; font-size: 1.5rem; margin: 1rem 0 0.5rem; color: #0B1F3B; }
