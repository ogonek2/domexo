<template>
    <div class="catalog-filters flex h-full flex-col bg-white">
        <div class="flex shrink-0 items-center justify-between bg-[#0B1F3B] px-4 py-3.5 text-white lg:px-5 lg:py-4">
            <div class="flex items-center gap-2.5">
                <span class="flex h-8 w-8 items-center justify-center bg-white/10">
                    <AppIcon name="sliders-horizontal" :size="16" />
                </span>
                <div>
                    <h2 class="font-heading text-sm font-bold leading-tight lg:text-base">Фільтри</h2>
                    <p v-if="activeCount > 0" class="text-[0.6875rem] text-white/60">
            ...
            <button
                type="button"
                class="flex h-8 w-8 items-center justify-center border border-white/20 transition-colors hover:bg-white/10 lg:hidden"
        ...
        <div class="flex min-h-0 flex-1 flex-col">
            <div class="flex-1 space-y-4 overflow-y-auto px-4 py-4 lg:space-y-5 lg:px-5 lg:py-5">
                <section v-if="categoryTree.length" class="catalog-filters__block">
                    ...
                    <button
                        ...
                        type="button"
                        class="mt-1.5 text-left text-xs font-medium text-[#D4AF5A] hover:text-[#C5A059]"
            ...
            <div class="catalog-filters__footer shrink-0 border-t border-gray-200 bg-white px-4 py-3 lg:px-5 lg:py-4">
                <p class="mb-2 text-center text-[0.6875rem] text-slate-400">Фільтри застосовуються одразу</p>
                <button
                    type="button"
                    class="flex w-full items-center justify-center gap-2 border border-gray-300 bg-white px-4 py-3 text-sm font-medium text-[#1E1E1E] transition-colors hover:border-[#0B1F3B] hover:text-[#0B1F3B]"
                    @click="$emit('reset')">
                    Скинути всі фільтри
                </button>
                <button
                    type="button"
                    class="mt-2 flex w-full items-center justify-center gap-2 bg-[#D4AF5A] px-4 py-3 font-heading text-sm font-semibold text-[#1E1E1E] transition-colors hover:bg-[#C5A059] lg:hidden"
                    @click="$emit('close')">
                    Готово
                </button>
            </div>
