...
@section('seo')
    <title>Результати пошуку: {{ $originalQuery }} — DOMEXO</title>
    <meta name="description" content="Результати пошуку товарів за запитом «{{ $originalQuery }}» в інтернет-магазині DOMEXO">
@endsection
    ...
    <nav aria-label="Breadcrumb" class="border-b border-gray-200 bg-white">
        <div class="mx-auto max-w-site px-4 py-6 sm:px-6 lg:px-8">
            <ol class="flex flex-wrap items-center gap-1.5 text-[0.8125rem] text-slate-500">
                <li>
                    <a href="{{ url('/') }}" class="transition-colors hover:text-[#0B1F3B]" aria-label="Головна">
                        <x-lucide-icon name="home" width="16" />
                    </a>
                </li>
                <li><x-lucide-icon name="chevron-right" width="14" class="text-gray-300" /></li>
                <li>
                    <span class="max-w-xs truncate font-medium text-[#1E1E1E]" aria-current="page">Пошук</span>
                </li>
            </ol>
        </div>
    </nav>
    ...
    <section class="border-b border-gray-200 bg-white">
        <div class="mx-auto max-w-site px-4 py-8 sm:px-6 md:py-10 lg:px-8">
            <div class="mx-auto max-w-3xl text-center">
                <div class="mx-auto mb-4 flex h-14 w-14 items-center justify-center border border-gray-200 bg-[#F5F6FB] text-[#0B1F3B]">
                    <x-lucide-icon name="search" width="26" />
                </div>
                <h1 class="font-heading text-2xl font-bold text-[#0B1F3B] md:text-3xl">
                    Результати пошуку
                </h1>
                <p class="mt-2 text-sm text-slate-500 md:text-base">
                    За запитом:
                    <span class="font-semibold text-[#1E1E1E]">«{{ $originalQuery }}»</span>
                </p>

                @if ($hasResults)
                    <p class="mt-3 text-sm text-slate-500">
                        Знайдено
                        <strong class="font-bold text-[#0B1F3B]">{{ $total }}</strong>
                ...
                @if ($usedAlternative && $query !== $originalQuery)
                    <div class="mt-4 inline-flex items-center gap-2 border border-[#D4AF5A]/40 bg-[#F5F0E6] px-4 py-2 text-sm text-[#1E1E1E]">
                        <x-lucide-icon name="info" width="16" class="text-[#D4AF5A]" />
            ...
            <form action="{{ route('search.index') }}" method="GET"
                ...
                <div class="flex min-w-0 flex-1 items-center gap-2.5 border border-gray-300 bg-white px-3.5 focus-within:border-[#D4AF5A] focus-within:shadow-[0_0_0_2px_rgba(212,175,90,0.18)]">
                    ...
                    <input id="searchPageQuery"
                           ...
                           class="min-w-0 flex-1 border-0 bg-transparent py-3 text-sm text-[#1E1E1E] outline-none placeholder:text-slate-400">
                </div>
                <button type="submit"
                        class="inline-flex items-center justify-center gap-2 bg-[#D4AF5A] px-6 py-3 font-heading text-sm font-semibold text-[#1E1E1E] transition-colors hover:bg-[#C5A059]">
    ...
    @if (!empty($suggestions))
        <section class="border-b border-gray-200 bg-white py-5">
            <div class="mx-auto max-w-site px-4 sm:px-6 lg:px-8">
                <h2 class="mb-3 flex items-center gap-2 font-heading text-sm font-semibold text-[#0B1F3B]">
                    <x-lucide-icon name="lightbulb" width="16" class="text-[#D4AF5A]" />
                    Можливо, ви мали на увазі:
                </h2>
                <div class="flex flex-wrap gap-2">
                    @foreach ($suggestions as $suggestion)
                        <a href="{{ url('/search?q=' . urlencode($suggestion)) }}"
                           class="inline-flex items-center border border-gray-300 bg-[#F5F6FB] px-3.5 py-2 text-sm text-[#1E1E1E] transition-colors hover:border-[#0B1F3B] hover:text-[#0B1F3B]">
                            «{{ $suggestion }}»
                        </a>
                    @endforeach
                </div>
    ...
    <section class="mx-auto max-w-site px-4 py-8 sm:px-6 md:py-12 lg:px-8">
        @if (!$hasResults)
            <div class="border border-gray-200 bg-white px-6 py-14 text-center sm:px-10">
                <div class="mx-auto mb-6 flex h-20 w-20 items-center justify-center border border-gray-200 bg-[#F5F6FB] text-gray-300">
                    <x-lucide-icon name="search-x" width="36" />
                </div>
                <h2 class="font-heading text-xl font-bold text-[#0B1F3B] md:text-2xl">Нічого не знайдено</h2>
                ...
                <div class="mx-auto mt-8 max-w-md border border-gray-200 bg-[#F5F6FB] p-5 text-left">
                    <h3 class="mb-3 flex items-center gap-2 font-heading text-sm font-semibold text-[#0B1F3B]">
                        <x-lucide-icon name="circle-check" width="16" class="text-[#D4AF5A]" />
                        Поради щодо пошуку
                    </h3>
                    <ul class="space-y-2 text-sm text-slate-600">
                        <li class="flex items-start gap-2">
                            <x-lucide-icon name="check" width="14" class="mt-0.5 shrink-0 text-[#D4AF5A]" />
                ...
                <a href="{{ route('catalog') }}"
                   class="mt-8 inline-flex items-center gap-2 bg-[#0B1F3B] px-6 py-3.5 font-heading text-sm font-semibold text-white transition-colors hover:bg-[#0a1830]">
        ...
        @else
            <div class="mb-5 flex flex-wrap items-center justify-between gap-4 border border-gray-200 bg-white px-5 py-4">
                <div>
                    <h2 class="font-heading text-base font-bold text-[#0B1F3B]">Знайдені товари</h2>
                    <p class="mt-0.5 text-sm text-slate-500">
                        Сторінка {{ $currentPage }} з {{ $lastPage }}
                    </p>
                </div>
                <a href="{{ route('catalog') }}"
                   class="inline-flex items-center gap-2 border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-[#1E1E1E] transition-colors hover:border-[#0B1F3B] hover:text-[#0B1F3B]">
                    <x-lucide-icon name="arrow-left" width="16" />
                    До каталогу
                </a>
            </div>
                    ...
                    <div class="flex flex-wrap items-center justify-center gap-1.5">
                        ...
                        @foreach ($visiblePages as $pageItem)
                            ...
                            @elseif ($pageItem['type'] === 'current')
                                <span class="inline-flex h-9 min-w-9 items-center justify-center border border-[#0B1F3B] bg-[#0B1F3B] px-2.5 text-sm font-bold text-white">
                                    {{ $pageItem['num'] }}
                                </span>
                            @else
